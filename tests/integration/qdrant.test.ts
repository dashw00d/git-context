...
async function testConnection(): Promise<boolean> {
  ...
  try {
    ...
    console.log(
      `   • Embedding Model: ${config.embeddingModel || 'openai/text-embedding-3-small (default)'}`
    );
    console.log(`   • Embedding Provider: ${config.embeddingProvider || 'not set'}`);
    console.log(
      `   • Per-Project Collections: ${config.perProjectQdrantCollections ? 'ENABLED' : 'DISABLED'}`
    );
...
async function testDefaultCollections(): Promise<boolean> {
  ...
  try {
    ...
    const config = getExtensionConfig();
    console.log(
      `   • Per-Project Collections setting: ${config.perProjectQdrantCollections ? 'ENABLED' : 'DISABLED'}`
    );
    ...
    if (commitsCollection !== expectedCommitsCollection) {
      console.log(
        `   ❌ Expected commits collection '${expectedCommitsCollection}', got '${commitsCollection}'`
      );
      if (originalValue) process.env.PER_PROJECT_QDRANT_COLLECTIONS = originalValue;
      return false;
    }
    ...
    if (symbolsCollection !== expectedSymbolsCollection) {
      console.log(
        `   ❌ Expected symbols collection '${expectedSymbolsCollection}', got '${symbolsCollection}'`
      );
      if (originalValue) process.env.PER_PROJECT_QDRANT_COLLECTIONS = originalValue;
      return false;
    }
    ...
    if (embedding.length !== qdrant.getEmbeddingDimension()) {
      console.log(
        `   ❌ Embedding dimension mismatch: expected ${qdrant.getEmbeddingDimension()}, got ${embedding.length}`
      );
      if (originalValue) process.env.PER_PROJECT_QDRANT_COLLECTIONS = originalValue;
      return false;
    }
...
async function testProjectCollections(): Promise<boolean> {
  ...
  try {
    ...
    if (config.perProjectQdrantCollections) {
      if (commitsCollection !== expectedCommitsCollection) {
        console.log(
          `   ⚠️  Expected commits collection '${expectedCommitsCollection}', got '${commitsCollection}'`
        );
        console.log(
          `   ⚠️  This may be because QdrantClientWrapper was initialized before config change`
        );
      } else {
      ...
      if (symbolsCollection !== expectedSymbolsCollection) {
        console.log(
          `   ⚠️  Expected symbols collection '${expectedSymbolsCollection}', got '${symbolsCollection}'`
        );
        console.log(
          `   ⚠️  This may be because QdrantClientWrapper was initialized before config change`
        );
    ...
    if (searchResults.length > 0) {
      searchResults.forEach((result, idx) => {
        ...
        if (payloadProjectId !== projectId) {
          console.log(
            `   ⚠️  Result ${idx} has wrong project_id: ${payloadProjectId} (expected ${projectId})`
          );
        }
      });
  ...
  try {
    ...
    try {
      ...
      if (error?.message?.includes('Bad Request')) {
        ...
        console.log(
          `   • Filter: ${JSON.stringify({ must: [{ key: 'project_id', match: { value: projectId } }] })}`
        );
      }
    }
...
async function testProjectIsolation(): Promise<boolean> {
  ...
  try {
    ...
    if (project2Results.length > 0) {
      console.log(
        `   ❌ Isolation broken! Found ${project2Results.length} project 2 results when searching for project 1`
      );
      return false;
    }
    ...
    if (project1Results2.length > 0) {
      console.log(
        `   ❌ Isolation broken! Found ${project1Results2.length} project 1 results when searching for project 2`
      );
      return false;
    }
