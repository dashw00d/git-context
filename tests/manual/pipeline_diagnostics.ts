...
function parseArgs(args: string[]): CliOptions {
  ...
  program
    ...
    .option(
      "--replay-step <stepId>",
      "Replay a specific step from frozen state"
    )
    .option(
      "--replay-from <stepId>",
      "Replay pipeline starting from a specific step"
    )
    .option("--validate-timeline", "Validate hybrid facts database consistency")
    .option(
      "--no-isolate",
      "Do not isolate analysis to /src (analyze all files)"
    )
    .option(
      "--real-llm",
      "Use real LLM calls instead of capturing prompts (costs money!)"
    )
  ...
  switch (stepId) {
    ...
    case "drift":
      ...
      return `missing=${data.missing_symbols?.length || 0}, zombies=${
        ...
        data.divergent_symbols?.length || 0
      }, hybrid=${hybridCount}`;
    case "legacy":
    ...
    case "workspace_overlay":
      ...
      if (
        data &&
        typeof data === "object" &&
        ("staged" in data || "unstaged" in data)
      ) {
      ...
      return `files=${data.filesChanged || 0}, symbols=${
        (data.symbolsAdded || 0) +
        (data.symbolsModified || 0) +
        (data.symbolsRemoved || 0)
      }`;
    case "index_commits":
    ...
    case "bundle_facts":
      const hybridFactsCount = data.hybridFacts
        ...
        : 0;
      const hybridFilesCount = data.hybridFacts
        ? Object.keys(data.hybridFacts).length
        : 0;
    ...
    case "embedding_index":
      ...
      return data.skipped
        ...
        : `commits=${data.commitCount || 0}, commitShards=${
          ...
          }, symbolShards=${data.symbolShardCount || 0}, themeShards=${
            data.themeShardCount || 0
          }`;
    case "hotspots":
      if (Array.isArray(data)) {
        const totalChurn = data.reduce(
          (sum: number, h: any) =>
            sum + (h.hotspotScore || h.churnScore || h.dnaChurn || 0),
          0
        );
        return `topHotspots=${data.length}, totalChurn=${totalChurn.toFixed(
          1
        )}`;
      }
  ...
  switch (stepId) {
      ...
      if (Array.isArray(data)) {
        const metrics = {
          ...
          totalEdges: data.reduce(
            (sum: number, c: any) =>
              sum + ((c.edgesAdded || 0) + (c.edgesRemoved || 0)),
            0
          ),
        };
    ...
    case "drift":
      return {
        ...
        conventionDrift: data.conventionDrift
          ? data.conventionDrift.driftSymbols?.length || 0
          : 0,
    ...
    case "hotspots":
      if (Array.isArray(data)) {
        return {
          ...
          withVersionTracking: data.filter((h: any) => h.touchedInVersions)
            .length,
    ...
    case "moved_blocks":
      if (Array.isArray(data)) {
        return {
          ...
          withVersionDescription: data.filter((m: any) => m.versionDescription)
            .length,
          moveTypes: {
    ...
    case "bundle_facts":
      ...
      return {
        ...
        hybridFactsCount,
        hybridFilesCount: data.hybridFacts
          ? Object.keys(data.hybridFacts).length
          : 0,
      };
...
class PromptCaptureLlmAnalyst extends LlmAnalyst {
  ...
  async analyze(
    ...
    const { AnalysisBlockUtils } = await import(
      "../../src/analysis/llmAnalyst/blocks"
    );
    ...
    const slimFacts = (this as any).buildSlimFacts
      ? (this as any).buildSlimFacts(facts)
      : facts;
    ...
    try {
      ...
      const promptTemplate = this.getPromptHelper(
        "intent",
        PROMPT_INTENT_AND_STORY
      );
      const timelineInfo = buildTimelineSummary(
        processedFacts.bundle?.timeline
      );
      ...
      const intentPrompt = this.buildPromptHelper(
        promptWithTimeline,
        processedFacts
      );
      ...
      const driftPromptTemplate = this.getPromptHelper(
        "drift",
        PROMPT_DRIFT_VERIFICATION
      );
      const driftTimelineInfo = buildTimelineSummary(
        processedFacts.bundle?.timeline
      );
      ...
      const driftPrompt = this.buildPromptHelper(
        driftPromptWithTimeline,
        processedFacts
      );
      this.capturedData.prompts.drift = driftPrompt;
      totalTokens += estimateTokens(driftPrompt);

      const driftBlock = AnalysisBlockUtils.createBlock(
        "drift",
        "Drift Verification",
        "drift"
      );
      ...
      const cleanupPromptTemplate = this.getPromptHelper(
        "cleanup",
        PROMPT_CLEANUP_PLAN
      );
      const cleanupTimelineInfo = buildTimelineSummary(
        processedFacts.bundle?.timeline
      );
      ...
      const cleanupPrompt = this.buildPromptHelper(
        cleanupPromptWithTimeline,
        processedFacts
      );
      this.capturedData.prompts.cleanup = cleanupPrompt;
      totalTokens += estimateTokens(cleanupPrompt);

      const cleanupBlock = AnalysisBlockUtils.createBlock(
        "cleanup",
        "Cleanup Plan",
        "cleanup"
      );
      ...
      if (rawFeed) {
        ...
        discoveryBlock = this.callParentMethod(
      ...
      const knownFiles = new Set(
        (facts.evidence["scope.files"] as string[]) || []
      );
          ...
          block.claims.reduce((claimAcc: number, claim: any) => {
            return (
              ...
              claim.evidence.filter(
                (e: any) => e.filePath && knownFiles.has(e.filePath)
              ).length
            );
          }, 0) +
  ...
  async discoverPatterns(
    ...
    // Discover prompt
    const discoverPromptTemplate = this.getPromptHelper(
      "discover",
      PROMPT_DISCOVER
    );
    ...
    const quantifyPromptTemplate = this.getPromptHelper(
      "quantify",
      PROMPT_QUANTIFY
    );
  ...
  private summarizeFactsHelper(
    ...
    if (
      summarized.evidence &&
      Array.isArray(summarized.evidence["working.symbols"])
    ) {
    ...
    if (
      summarized.evidence &&
      Array.isArray(summarized.evidence["working.edges"])
    ) {
      ...
      summarized.evidence["working.edges"] = Array.from(
        edgeCounts.entries()
    ...
    for (const key in summarized.evidence) {
      if (
        ...
        summarized.evidence[key] = summarized.evidence[key].slice(
          0,
          maxEvidenceItems
        );
      }
...
async function main() {
  ...
  const { auditAllModules, MODULE_SCHEMAS } = await import(
    "../../src/storage/schema"
  );
  const auditGaps = auditAllModules(db);
  console.log(
    `📊 Schema: ${Object.keys(MODULE_SCHEMAS).length} modules, audit gaps: ${
      auditGaps.length
    }`
  );
  ...
  if (!options.noIsolate) {
    ...
    console.log(
      "🔧 Ignore paths: isolating to src/** (all other top-level paths ignored)"
    );
  } else {
    console.log(
      "🔧 Ignore paths: standard .gitignore only (no extra isolation)"
    );
  }
  ...
  db.transaction(() => {
    for (const commit of commitsWithFiles) {
      if (!commit.author) {
        console.warn(
          `⚠️ Missing author for ${commit.sha}, defaulting to 'Unknown'`
        );
      }
  ...
  console.log(
    `📋 Analyzing ${shas.length} commits (HEAD: ${shas[0].substring(0, 8)})`
  );
  ...
  const explicitTimeline = buildExplicitTimeline({
    includeUnstaged:
      options.includeWorkspace && (workspaceParts?.has("unstaged") ?? true),
    includeStaged:
      options.includeWorkspace && (workspaceParts?.has("staged") ?? true),
  ...
  let replayIndex = replayAnchor
    ? steps.findIndex((s) => s.id === replayAnchor)
    : -1;
  if (replayAnchor && replayIndex === -1) {
    console.warn(
      `⚠ Replay target '${replayAnchor}' not found; running full pipeline`
    );
  } else if (replayIndex >= 0) {
    const snap = loadStepSnapshot(replayAnchor, options.snapshotDir);
    if (!snap) {
      throw new Error(
        `Snapshot for '${replayAnchor}' not found at ${options.snapshotDir}`
      );
    }
  ...
  const onEvent = (event: any) => {
    if (event.type === "start") {
      ...
      const metrics = options.fullReport
        ? extractStepMetrics(stepId, fullData)
        : {};
      ...
      if (
        stepId === "embedding_index" ||
        stepId === "llm_story" ||
        stepId === "retrieve_history"
      ) {
      ...
      if (loadTime > 10) {
        console.warn(
          `[Diagnostics] Slow snapshot load for ${stepId}: ${loadTime}ms`
        );
      }
      ...
      if (stepId === "llm_story") {
        ...
        const capturedData = (
          llmAnalyst as PromptCaptureLlmAnalyst
        ).getCapturedData();
        ...
        if (capturedData.summarizedFacts) {
          ...
          fs.writeFileSync(
            factsPath,
            JSON.stringify(capturedData.summarizedFacts, null, 2)
          );
          console.log(
        ...
        if (promptFiles.length > 0) {
          ...
          promptFiles.forEach(({ phase, path, size }) => {
            console.log(
              `   ${phase}: ${(size / 1024).toFixed(1)}KB -> ${path}`
            );
          });
        }
        ...
        if (event.state.llmOutputs) {
          ...
          if (llmAnalysis) {
            llmSummary = {
              ...
              validatedEvidenceCount:
                llmAnalysis.metadata?.validatedEvidenceCount,
              ...
              prompts: {
                ...
                quantify: capturedData.prompts.quantify
                  ? { size: capturedData.prompts.quantify.length }
                  : null,
                plan: capturedData.prompts.plan
                  ? { size: capturedData.prompts.plan.length }
                  : null,
              },
    ...
    if (driftIndex >= 0) {
      ...
      steps[driftIndex] = {
        ...
        run: async (state: any) => {
          ...
          diagnostics.performance.driftDetectionTime =
            driftEndTime - driftStartTime;
        },
      };
    }
  }
  ...
  if (finalState.bundleFacts?.hybridFacts) {
    ...
    for (const [filePath, facts] of Object.entries(hybridFacts)) {
      ...
      const hasSemanticSymbols = factArray.some(
        (f: any) =>
          f.kind &&
          !["cst_node", "heading", "property", "doc_comment"].includes(f.kind)
      );
  ...
  if (options.fullReport) {
    const indexCommitsStep = diagnostics.steps.find(
      (s: any) => s.stepId === "index_commits"
    );
    const workingStep = diagnostics.steps.find(
      (s: any) => s.stepId === "working"
    );
    const intendedStep = diagnostics.steps.find(
      (s: any) => s.stepId === "intended"
    );
    const driftStep = diagnostics.steps.find((s: any) => s.stepId === "drift");
    const hotspotsStep = diagnostics.steps.find(
      (s: any) => s.stepId === "hotspots"
    );
    const bundleFactsStep = diagnostics.steps.find(
      (s: any) => s.stepId === "bundle_facts"
    );
    const legacyStep = diagnostics.steps.find(
      (s: any) => s.stepId === "legacy"
    );
    const embeddingStep = diagnostics.steps.find(
      (s: any) => s.stepId === "embedding_index"
    );
    const historyStep = diagnostics.steps.find(
      (s: any) => s.stepId === "retrieve_history"
    );
    const llmStep = diagnostics.steps.find(
      (s: any) => s.stepId === "llm_story"
    );

    diagnostics.aggregatedMetrics = {
      totalCommits: indexCommitsStep?.metrics?.commitCount || 0,
      totalSymbols:
        (workingStep?.metrics?.symbols || 0) +
        (intendedStep?.metrics?.totalSymbols || 0),
      ...
      llmTotalCalls:
        diagnostics.llm_summaries?.reduce(
          (sum: number, l: any) => sum + (l.totalCalls || 0),
          0
        ) || 0,
      llmTotalTokens:
        diagnostics.llm_summaries?.reduce(
          (sum: number, l: any) => sum + (l.totalTokens || 0),
          0
        ) || 0,
      overallHealthScore:
    ...
    if (finalState.drift) {
      ...
      diagnostics.finalStates.drift = {
        ...
        conventionDrift: finalState.drift.conventionDrift
          ? {
              dominantConvention:
                finalState.drift.conventionDrift.dominantConvention,
              driftPercent: finalState.drift.conventionDrift.driftPercent,
              driftSymbolCount:
                finalState.drift.conventionDrift.driftSymbols?.length || 0,
            }
          : undefined,
      };
    }
  ...
  if (diagnostics.performance.driftDetectionTime > 0) {
    const speedup =
      diagnostics.performance.speedup > 1
        ? ` (${diagnostics.performance.speedup.toFixed(
            1
          )}x faster with batching)`
        : "";
    console.log(
  ...
  if (options.fullReport) {
    ...
    diagnostics.steps.forEach((step: any) => {
      ...
      if (step.llmSummary) {
        ...
        if (step.llmSummary.summary_text) {
          reportMd += `  - Summary: ${step.llmSummary.summary_text.substring(
            0,
            200
          )}...\n`;
        }
    ...
    const hotspots =
      diagnostics.steps.find((s: any) => s.stepId === "hotspots")?.metrics
        ?.topHotspots || 0;
    ...
    if (hotspots > 0 || driftIssues > 0) {
      ...
      if (driftIssues > 0) {
        ...
        const driftStep = diagnostics.steps.find(
          (s: any) => s.stepId === "drift"
        );
    ...
    // Final summary
    reportMd += `## Summary\n\n`;
    reportMd += `- **Total Steps:** ${
      diagnostics.completedSteps?.length || 0
    }\n`;
    reportMd += `- **Errors:** ${diagnostics.errorCount || 0}\n`;
    if (diagnostics.aggregatedMetrics) {
      reportMd += `- **Total Symbols:** ${
        diagnostics.aggregatedMetrics.totalSymbols || 0
      }\n`;
      reportMd += `- **Total Edges:** ${
        diagnostics.aggregatedMetrics.totalEdges || 0
      }\n`;
      reportMd += `- **Drift Issues:** ${
        diagnostics.aggregatedMetrics.totalDriftIssues || 0
      }\n`;
    }
    reportMd += `\n`;
...
async function validateHybridFactsDatabase(
  ...
  try {
    ...
    const unstagedCount = db
      .prepare(
        'SELECT COUNT(*) as count FROM hybrid_facts WHERE version="workspace-unstaged"'
      )
      .get() as any;
    const stagedCount = db
      .prepare(
        'SELECT COUNT(*) as count FROM hybrid_facts WHERE version="workspace-staged"'
      )
      .get() as any;
    const workspaceCount = db
      .prepare(
        'SELECT COUNT(*) as count FROM hybrid_facts WHERE version="workspace"'
      )
      .get() as any;
    const headCount = db
      .prepare(
        'SELECT COUNT(*) as count FROM hybrid_facts WHERE version="HEAD"'
      )
      .get() as any;
      ...
      if (sampleFiles.length > 0) {
        ...
        for (const { file_path } of sampleFiles) {
          for (const version of explicitTimeline) {
            try {
              const facts = await timelineManager.getPriorFacts(
                file_path,
                version
              );
        ...
        if (sampleFiles.length > 0 && explicitTimeline.length > 0) {
          ...
          try {
            const batchFacts = await timelineManager.getPriorFactsBatch(
              testFiles,
              testVersion
            );
            console.log(
