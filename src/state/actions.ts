...
export type Action =
    ...
    | { type: 'WORKSPACE_FILES_UPDATED'; payload: { staged: StagedFileDTO[]; unstaged: UnstagedFileDTO[] } }

    // UI State
    | { type: 'SECTION_CHANGED'; payload: { section: CockpitSectionKey } }
    | { type: 'COMMITS_FILTER_TEXT_CHANGED'; payload: { text: string } }
    | { type: 'COMMITS_FILTER_SCOPES_CHANGED'; payload: { scopes: { staged: boolean; unstaged: boolean; history: boolean } } }
    ...
    // Context
    | { type: 'REPO_CONTEXT_UPDATED'; payload: { repoName: string | null; branchName: string | null } }

    // Legacy / Migration
    | { type: 'LEGACY_STATE_UPDATED'; payload: { partial: any; reason?: string } }
    | { type: 'REFRESH_REQUESTED'; payload: { scope: 'all' | 'commits' | 'bundle' | 'symbols' | 'reports' } }
