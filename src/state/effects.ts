...
export class CockpitEffects {
    ...
    private async refreshCommits() {
        ...
        const commits = await this.providers.commitsProvider.exportCommitsDto(effectiveLimit + this.providers.commitsProvider.loadMoreOffset);
        ...
        const analyzedStatuses = await Promise.all(
            commits.map(async (commit: any) => {
                if (!db) return { sha: commit.sha, analyzed: false };
        ...
        const commitDtos = commits.map((commit: any) => {
            const files = Array.isArray(commit.files)
                ? commit.files.map((file: any) => ({ path: file.path, status: this.mapStatus(file.status) }))
                : undefined;
        ...
        this.store.dispatch({ type: 'COMMITS_UPDATED', payload: { commits: commitDtos, hasMore } });
    }
    ...
    private async refreshWorkspace() {
        ...
        const staged = workspaceFiles.staged.map((file: any) => ({ path: file.path, status: this.mapStatus(file.status) }));
        const unstaged = workspaceFiles.unstaged.map((file: any) => ({ path: file.path, status: this.mapStatus(file.status) }));

        this.store.dispatch({ type: 'WORKSPACE_FILES_UPDATED', payload: { staged, unstaged } });
    }
    ...
    private async refreshBundle() {
        ...
        if (bundleFacts) {
            ...
            this.store.dispatch({ type: 'LEGACY_STATE_UPDATED', payload: { partial: { bundleFacts, bundleSummary: summary } } });
        }
    }
    ...
    private async refreshSymbols() {
        ...
        this.store.dispatch({ type: 'SYMBOLS_UPDATED', payload: { symbols: symbolDtos } });
    }
    ...
    private async handleAnalysis(payload: { selection: string[]; force?: boolean }) {
        ...
        try {
            ...
            const explicitSelection = payload.selection && payload.selection.length > 0
                ? payload.selection
            ...
            this.store.dispatch({ type: 'SELECTION_SET', payload: { shas: ordered } });
            ...
            this.store.dispatch({ type: 'ANALYSIS_FAILED', payload: { error: String(error) } });
        }
    }
}
