...
export class ExplorerService {
     ...
     */
    getExplorerTree(bundleFacts: BundleFactsDTO | null, skeleton: { files: string[] } | null, bundles: any[] = []): ExplorerNode[] {
    ...
    private buildFileTree(files: string[], defaultStatus: 'scanning' | 'ready'): ExplorerNode[] {
        console.log(`[ExplorerService] Building tree for ${files.length} files (status=${defaultStatus})`);
        if (files.length > 0) {
            console.log(`[ExplorerService] Sample files: ${files.slice(0, 3).join(', ')}`);
        }
        const root: ExplorerNode = { id: 'root', name: 'src', type: 'folder', status: 'ready', children: [] };
