...
export class FrameAnalyzer {
    ...
    async analyzeTier2(
        ...
        if (facts) {
            ...
            const hotspots = (facts.evidence?.hotspots || (facts.findings as any)?.hotspots || []) as any[];
        ...
        try {
            ...
            const virtualStaged = await gitOps.getDiffStats('staged').catch(() => ({ added: 0, removed: 0 }));
            const virtualUnstaged = await gitOps.getDiffStats('unstaged').catch(() => ({ added: 0, removed: 0 }));

            const timeline = [
                ...(virtualUnstaged.added || virtualUnstaged.removed ? [{ message: 'Unstaged changes', author: 'workspace', date: new Date().toISOString(), virtual: true, stats: virtualUnstaged }] : []),
                ...(virtualStaged.added || virtualStaged.removed ? [{ message: 'Staged changes', author: 'workspace', date: new Date().toISOString(), virtual: true, stats: virtualStaged }] : []),
                ...gitHistory
            ];

            const latestStats = gitHistory.find((h: any) => h.stats) ? gitHistory[0].stats : { additions: 0, deletions: 0 };
            const changeStats = {
                added: (virtualStaged.added || 0) + (virtualUnstaged.added || 0) + (latestStats?.additions || 0),
                removed: (virtualStaged.removed || 0) + (virtualUnstaged.removed || 0) + (latestStats?.deletions || 0)
            };
