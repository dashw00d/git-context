...
export const StatsSection: React.FC<StatsSectionProps> = ({ state, vscode }) => {
    const selectionCount = state.selectedCommitShas.length +
    ...
    const criticalCount = (findings?.incompleteness?.missing || 0) +
        (findings?.incompleteness?.zombies || 0) +
        (findings?.legacyAudit?.dead || 0);
    const warningCount = (findings?.legacyAudit?.legacyUsed || 0) +
        ...
        <div className="cockpit__stats-section">
            <div className="cockpit__stat-group">
                ...
                <div className="cockpit__stat-list">
                    {state.selectedStagedPaths.length > 0 && <span className="tag">Staged ({state.selectedStagedPaths.length})</span>}
                    {state.selectedUnstagedPaths.length > 0 && <span className="tag">Unstaged ({state.selectedUnstagedPaths.length})</span>}
                    {state.selectedCommitShas.slice(0, 3).map(sha => (
                        <span key={sha} className="tag">{sha.substring(0, 7)}</span>
                    ))}
                    {state.selectedCommitShas.length > 3 && <span className="tag">+{state.selectedCommitShas.length - 3}</span>}
                </div>
            </div>
            ...
            <div className="cockpit__stat-group">
                ...
                <div className="cockpit__stat-row">
                    <div className="cockpit__stat-item">
                        <span className="value">{bundleSummary?.commitCount || state.selectedCommitShas.length}</span>
                ...
                {state.bundleReportId && (
                    <button
                        ...
                        onClick={() => vscode.postMessage({ type: 'openReport', reportId: state.bundleReportId })}
                    >
                        Open Full Report
                    </button>
                )}
