...
export const SymbolList: React.FC<SymbolListProps> = ({ state, vscode }) => {
  ...
  const list = state.symbols.filter(
    s =>
      ...
      (state.symbolChangeFilter === 'all' ||
        (s.changeType || 'modified') === state.symbolChangeFilter)
  );

  if (!list.length) {
    const hasFilters =
      state.symbolFilterText ||
      state.symbolKindFilter !== 'all' ||
      state.symbolChangeFilter !== 'all';
    return (
      ...
      {list.map(s => (
        <li key={s.id} className="cockpit__list-item">
          <div className="cockpit__row">
            <span
              className="cockpit__codicon"
              data-icon={getSymbolKindIcon(s.kind)}
              title={s.kind}
            ></span>
            <span>{s.name}</span>
            {s.changeType && (
