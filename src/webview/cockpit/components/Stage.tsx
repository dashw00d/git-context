...
const TreemapNode: React.FC<{ node: any; depth?: number }> = ({ node, depth = 0 }) => {
    ...
    return (
        <div style={{
            ...
            padding: 6,
            margin: 4,
            background: hasChildren ? 'var(--vscode-editor-background)' : 'var(--vscode-editor-inactiveSelectionBackground)',
            display: 'flex',
            flexDirection: 'column',
            gap: 4
        }} title={`${node.name}\nChurn: ${node.score.toFixed(1)}\n+${node.added || 0} / -${node.removed || 0}`}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8em', fontWeight: 'bold', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                <span>{node.name}</span>
                <span style={{ color: baseColor }}>{node.score}</span>
            </div>
            <div style={{ height: 4, borderRadius: 2, background: 'var(--vscode-progressBar-background)', overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${Math.min((node.score || 0) * 2, 100)}%`, background: baseColor }} />
            </div>
...
const BundleStage: React.FC<{ frame: any, onZoomIn: (frame: any) => void, cockpitState?: CockpitState, vscode?: any }> = ({ frame, onZoomIn, cockpitState, vscode }) => {
    const [isConfigOpen, setIsConfigOpen] = React.useState(false);
    type BundleConfig = { mode: 'repo' | 'module' | 'changes' | 'custom'; roots: string[]; includeConnected: boolean; exclusions: string[] };
    const defaultConfig: BundleConfig = { mode: 'repo', roots: [], includeConnected: false, exclusions: [] };
    const [formConfig, setFormConfig] = React.useState<BundleConfig>(cockpitState?.bundleConfig || defaultConfig);
    const [depth, setDepth] = React.useState<number>(cockpitState?.lastNCommits || 20);

    React.useEffect(() => {
        setFormConfig(cockpitState?.bundleConfig || { mode: 'repo', roots: [], includeConnected: false, exclusions: [] });
        ...
        <div style={{ width: '100%', padding: '10px', overflow: 'auto' }}>
            ...
            {isConfigOpen && (
                <div style={{
                    ...
                    <div style={{ marginBottom: '10px' }}>
                        <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '0.9em' }}>Mode</label>
                        <select
                            value={formConfig.mode}
                            onChange={(e) => handleConfigChange({ mode: e.target.value as BundleConfig['mode'] })}
                            style={{ width: '100%', padding: '4px', background: 'var(--vscode-dropdown-background)', color: 'var(--vscode-dropdown-foreground)', border: '1px solid var(--vscode-dropdown-border)' }}
                        >
                    ...
                    {formConfig.mode === 'custom' && (
                        <div style={{ marginBottom: '10px' }}>
                            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '0.9em' }}>Roots (comma separated)</label>
                            <input
                                type="text"
                                value={formConfig.roots.join(', ')}
                                onChange={(e) => handleConfigChange({ roots: e.target.value.split(',').map(s => s.trim()).filter(Boolean) })}
                    ...
                    <div style={{ marginBottom: '10px' }}>
                        <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '0.9em' }}>Exclusions (glob)</label>
                        <input
                            type="text"
                            value={formConfig.exclusions.join(', ')}
                            onChange={(e) => handleConfigChange({ exclusions: e.target.value.split(',').map(s => s.trim()).filter(Boolean) })}
                            placeholder="**/*.test.ts"
                            style={{ width: '100%', padding: '4px', background: 'var(--vscode-input-background)', color: 'var(--vscode-input-foreground)', border: '1px solid var(--vscode-input-border)' }}
                        />
                    </div>
                    ...
                    <div style={{ marginBottom: '10px' }}>
                        <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '0.9em' }}>Analysis Depth (Commits)</label>
                        <input
                            type="number"
                            min="1"
                            max="100"
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                        <button
                            ...
                            style={{
                                flex: 1,
                                padding: '6px',
                                background: cockpitState?.isAnalyzing ? 'var(--vscode-button-secondaryBackground)' : 'var(--vscode-button-background)',
                                color: cockpitState?.isAnalyzing ? 'var(--vscode-button-secondaryForeground)' : 'var(--vscode-button-foreground)',
                                border: 'none',
                                borderRadius: '2px',
            ...
            {cockpitState?.bundleSummary && (
                <div style={{
                    ...
                    <div><strong>{cockpitState.bundleSummary.commitCount}</strong> commits</div>
                    <div><strong>{cockpitState.bundleSummary.fileCount}</strong> files</div>
                    {cockpitState.bundleSummary.symbolCount && <div><strong>{cockpitState.bundleSummary.symbolCount}</strong> symbols</div>}
                </div>
            )}
            ...
            {frame.data?.treemap && frame.data.treemap.length > 0 ? (
                ...
                <p style={{ fontSize: '0.85em', color: 'var(--vscode-descriptionForeground)' }}>
                    {cockpitState?.isAnalyzing ? 'Loading heatmap...' : 'No churn data yet. Run Analyze to populate the heatmap.'}
                </p>
            )}
            ...
            {frame.data?.hotspots && frame.data.hotspots.length > 0 && (
                <>
                    <p style={{ fontSize: '0.85em', color: 'var(--vscode-descriptionForeground)', marginBottom: '10px' }}>
                            ...
                            >
                                ...
                                <div style={{
                                    ...
                                    <div style={{
                                        ...
                                        height: '100%',
                                        background: file.score > 10 ? 'var(--vscode-charts-red)' : file.score > 5 ? 'var(--vscode-charts-orange)' : 'var(--vscode-charts-green)'
                                    }}></div>
                                </div>
                            </div>
                        ))}
                ...
                <div style={{ marginTop: '20px' }}>
                    ...
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                        {frame.data.risks.map((risk: any, idx: number) => (
                            <li key={idx} style={{ padding: '8px', border: '1px solid var(--vscode-inputValidation-warningBorder)', borderRadius: '4px', background: 'var(--vscode-inputValidation-warningBackground)', marginBottom: '8px' }}>
...
const BlastRadiusStage: React.FC<{ frame: any, onZoomIn: (frame: any) => void }> = ({ frame, onZoomIn }) => {
    ...
    const edges = [
        ...incoming.map((e: any) => ({ from: e.from.split(':')[0], to: center, direction: 'in', type: e.type })),
        ...outgoing.map((e: any) => ({ from: center, to: e.to.split(':')[0], direction: 'out', type: e.type }))
    ];
    return (
        <div style={{ width: '100%', height: '100%', padding: '10px', overflow: 'auto', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '6px' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85em' }}>
                    <input type="checkbox" checked={showIncoming} onChange={e => setShowIncoming(e.target.checked)} /> Incoming
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85em' }}>
                    <input type="checkbox" checked={showOutgoing} onChange={e => setShowOutgoing(e.target.checked)} /> Outgoing
                </label>
            </div>
            <div style={{ border: '1px solid var(--vscode-panel-border)', borderRadius: '4px', padding: '8px', minHeight: 180 }}>
                <div style={{ position: 'relative', width: '100%', height: 200 }}>
                    ...
                    {uniqueNodes.map((n, idx) => {
                        ...
                        return (
                            <div
                                key={n.id}
                                onClick={() => onZoomIn({ level: 'file', id: n.id, name: n.id.split('/').pop(), status: 'scanning' })}
                                style={{
                                    position: 'absolute',
                                    top: `${y}%`,
                                    left: `${x}%`,
                                    ...
                                    padding: '4px 6px',
                                    borderRadius: '6px',
                                    background: n.direction === 'in' ? 'var(--vscode-charts-blue)' : 'var(--vscode-charts-orange)',
                    ...
                    {edges
                        .filter(e => (e.direction === 'in' && showIncoming) || (e.direction === 'out' && showOutgoing))
                        .map((e, idx) => {
                            ...
                            return (
                                <svg key={idx} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none' }}>
                                    <line x1={`${x1}%`} y1={`${y1}%`} x2={`${x2}%`} y2={`${y2}%`} stroke={e.direction === 'in' ? 'var(--vscode-charts-blue)' : 'var(--vscode-charts-orange)'} strokeWidth={1.5} opacity={0.6} />
                                </svg>
                            );
                        })}
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', flex: 1 }}>
                <div style={{ border: '1px solid var(--vscode-panel-border)', borderRadius: '4px', padding: '8px', overflow: 'auto' }}>
                    <div style={{ fontWeight: 'bold', marginBottom: '6px' }}>Incoming ({incoming.length})</div>
                    {incoming.length ? (
                        <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                            {incoming.map((edge: any, idx: number) => (
                                <li
                                    ...
                                    style={{ padding: '6px', borderBottom: '1px solid var(--vscode-panel-border)', cursor: 'pointer', fontSize: '0.85em' }}
                                >
                ...
                <div style={{ border: '1px solid var(--vscode-panel-border)', borderRadius: '4px', padding: '8px', overflow: 'auto' }}>
                    <div style={{ fontWeight: 'bold', marginBottom: '6px' }}>Outgoing ({outgoing.length})</div>
                    {outgoing.length ? (
                        <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
...
const FileStage: React.FC<{ frame: any, onZoomIn: (frame: any) => void }> = ({ frame, onZoomIn }) => {
    const [showSymbols, setShowSymbols] = React.useState(false);

    return (
        <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '10px', borderBottom: '1px solid var(--vscode-panel-border)', flexShrink: 0 }}>
                ...
                <div style={{ fontSize: '0.8em', opacity: 0.8 }}>
                    ...
                    {frame.data?.changeStats && (
                        <span style={{ marginLeft: '10px' }}>+{frame.data.changeStats.added} / -{frame.data.changeStats.removed}</span>
                    )}
                    {typeof frame.data?.hotspotScore === 'number' && (
                        <span style={{ marginLeft: '10px', color: 'var(--vscode-charts-red)' }}>Hotspot: {frame.data.hotspotScore}</span>
                    )}
                </div>
            </div>
            ...
            <div style={{ flex: 1, overflow: 'hidden', display: 'flex' }}>
                {!showSymbols ? (
                    <div style={{ flex: 1, overflow: 'auto', backgroundColor: 'var(--vscode-editor-background)', padding: '10px' }}>
                        <pre style={{
                            margin: 0,
                        ...
                        <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                            {frame.data?.symbols?.map((symbol: any) => (
                                ...
                                >
                                    ...
                                    {symbol.signature && (
                                        <div style={{ fontSize: '0.75em', opacity: 0.7, marginTop: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                            {symbol.signature}
                                        </div>
                                    )}
                                </li>
                            ...
                            {!frame.data?.symbols?.length && (
                                <li style={{ padding: '20px', opacity: 0.6, textAlign: 'center' }}>No symbols found</li>
                            )}
                        </ul>
                    </div>
                )}
...
const SymbolStage: React.FC<{ frame: any, vscode?: any }> = ({ frame, vscode }) => (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '10px', borderBottom: '1px solid var(--vscode-panel-border)', flexShrink: 0 }}>
            ...
            <div style={{ marginTop: '8px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                ...
                {frame.data?.drift?.[0]?.suggestedName && vscode && (
                    <button
                        onClick={() => {
                            ...
                            vscode.postMessage({ type: 'applyRefactorSuggestion', payload: { symbolId: frame.data.symbolId, suggestedName: suggestion, filePath: frame.data.filePath } });
                        }}
                        style={{
                            padding: '4px 8px',
                            border: '1px solid var(--vscode-button-border)',
                ...
                {vscode && (
                    <button
                        onClick={() => {
                            vscode.postMessage({ type: 'askAssistant', payload: { symbolId: frame.data?.symbolId, filePath: frame.data?.filePath, drift: frame.data?.drift } });
                        }}
                        style={{
                            padding: '4px 8px',
                            border: '1px solid var(--vscode-button-border)',
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            {/* Code */}
            <div style={{ flex: 1, overflow: 'auto', backgroundColor: 'var(--vscode-editor-background)', padding: '10px', borderBottom: '1px solid var(--vscode-panel-border)' }}>
                <pre style={{
                    margin: 0,
                    fontFamily: 'var(--vscode-editor-font-family)',
                    fontSize: '0.85em',
            ...
            <div style={{ maxHeight: '220px', overflow: 'auto', padding: '10px', flexShrink: 0 }}>
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                    {frame.data?.history?.map((commit: any) => (
                        <li key={commit.hash || commit.message || commit.date || commit.author} style={{ marginBottom: '12px', borderLeft: '2px solid var(--vscode-charts-blue)', paddingLeft: '8px' }}>
                            <div style={{ fontWeight: 'bold', fontSize: '0.85em' }}>{commit.message || commit.summary || commit.change}</div>
                            <div style={{ fontSize: '0.75em', opacity: 0.7, marginTop: '2px' }}>
                                {commit.date ? new Date(commit.date).toLocaleDateString() : ''} • {commit.author || 'unknown'}
                            </div>
                        </li>
                    ))}
...
export const Stage: React.FC<StageProps & { cockpitState?: CockpitState; vscode?: any }> = ({ frame, onZoomIn, onZoomOut, cockpitState, vscode }) => {
                ...
                {frame.status === 'scanning' ? (
                    ...
                    <>
                        {frame.id === 'reports-root' && cockpitState && vscode ? (
                            ...
                            <>
                                {frame.level === 'bundle' && <BundleStage frame={frame} onZoomIn={(f) => onZoomIn(f)} cockpitState={cockpitState} vscode={vscode} />}
                                {frame.level === 'blast_radius' && <BlastRadiusStage frame={frame} onZoomIn={(f) => onZoomIn(f)} />}
