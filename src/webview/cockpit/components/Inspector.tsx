    ...
    <div style={InspectorContainer}>
      ...
      <div style={ContentStyle}>
            ...
            <ul style={{ paddingLeft: '20px', marginTop: '10px', listStyle: 'none' }}>
              {timeline.map((item: any, index: number) => (
                <li
                  ...
                  style={{
                    marginBottom: '15px',
                    borderLeft: item.virtual
                      ? '2px dashed var(--vscode-gitDecoration-modifiedResourceForeground)'
                  ...
                  {frame.data.history.map((h: any, idx: number) => (
                    ...
                    >
                      ...
                      <div style={{ opacity: 0.7 }}>
                        {h.date ? new Date(h.date).toLocaleString() : ''} • {h.author || 'unknown'}
                      </div>
                    </li>
                  ))}
                </ul>
          ...
          <div>
            {drift.length > 0 ? (
              ...
              <div style={{ padding: '10px', opacity: 0.7 }}>
                <span style={{ color: 'var(--vscode-testing-iconPassed)' }}>✔</span> No drift
                detected.
              </div>
            )}
        ...
        {activeLens === 'relations' && (
          <div>
            <p>
              <strong>Incoming:</strong> {relations.incoming?.length || 0} callers
            </p>
            {relations.incoming?.length > 0 && (
              <ul style={{ listStyle: 'none', padding: 0, margin: '6px 0 10px 0' }}>
                {relations.incoming.map((edge: any, idx: number) => (
                  ...
                  >
                    {edge.from?.split(':')[0] || edge.from} → {edge.to?.split(':')[0] || edge.to} (
                    {edge.type})
                  </li>
                ))}
              </ul>
            ...
            <p>
              <strong>Outgoing:</strong> {relations.outgoing?.length || 0} callees
            </p>
            ...
            <p>
              <strong>Imports:</strong> {relations.imports?.length || 0} modules
            </p>
          </div>
        )}
            ...
            {risk ? (
              ...
              >
                ...
                <p style={{ margin: '5px 0 0 0', fontSize: '0.9em' }}>
                  Hotspot: {risk.hotspotScore} • Drift: {risk.driftCount} • Legacy:{' '}
                  {risk.legacyDead}/{risk.legacyReplaced}
                </p>
              ...
              <div style={{ padding: '10px', opacity: 0.7 }}>
                <span style={{ color: 'var(--vscode-testing-iconPassed)' }}>✔</span> No risk
                signals.
              </div>
            )}
                  ...
                  {frame.data.drift.map((d: any, idx: number) => (
                    ...
                    >
                      ...
                      <div style={{ fontSize: '0.8em', opacity: 0.8 }}>
                        {d.issue || d.detail || d.suggestedName || 'Drift detected'}
                      </div>
                    </li>
                  ))}
                </ul>
