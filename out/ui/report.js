...
exports.generateCommitReport = exports.generateRefactorBundleReport = void 0;
 ...
 */
async function computeScope(commitShas) {
 ...
 */
async function computeBlastRadiusNeighbors(commitShas, commitFiles, maxNeighbors) {
    const { getDatabaseManager } = await Promise.resolve().then(() => __importStar(require('../storage/database')));
    const db = getDatabaseManager().getDatabase();
    const neighborFiles = new Map(); // file -> confidence score
    ...
    LIMIT 200
  `);
    const edges = edgesStmt.all(...commitShas);
 ...
 */
async function getScopedWorkingTreeSnapshot(scopePaths) {
 ...
 */
function detectWideningNeeds(intended, working, commitShas) {
 ...
 */
function checkForExternalCallers(symbolId, commitShas, analyzedPaths) {
 ...
 */
function getCallerFiles(symbolId, commitShas) {
 ...
 */
function checkForUnknownDestinations(symbolId, working, commitShas) {
 ...
 */
function getDestinationFiles(symbolId, commitShas) {
 ...
 */
async function buildIntendedRefactorMap(commitShas) {
    const { getDatabaseManager } = await Promise.resolve().then(() => __importStar(require('../storage/database')));
    const db = getDatabaseManager().getDatabase();
    ...
    ORDER BY date ASC
  `);
    const orderedShas = shaOrderStmt.all(...commitShas).map((row) => row.sha);
    ...
    `);
        const renames = renamesStmt.all(sha);
 ...
 */
function detectRefactorDrift(intended, working) {
    const findings = {
        missing_symbols: [],
        zombie_symbols: [],
        divergent_symbols: [],
 ...
 */
async function generateRefactorBundleReport(commitShas) {
    try {
        vscode.window.withProgress({
            ...
            let widening = { needsWidening: false, additionalPaths: new Set() };
            ...
            if (drift.missing_symbols.length > 0) {
                ...
                if (drift.missing_symbols.length > 10)
                    markdown += `- ... and ${drift.missing_symbols.length - 10} more\n`;
                markdown += `\n`;
            }
            if (drift.zombie_symbols.length > 0) {
                ...
                if (drift.zombie_symbols.length > 10)
                    markdown += `- ... and ${drift.zombie_symbols.length - 10} more\n`;
                markdown += `\n`;
            }
            if (drift.divergent_symbols.length > 0) {
                ...
                if (drift.divergent_symbols.length > 10)
                    markdown += `- ... and ${drift.divergent_symbols.length - 10} more\n`;
                markdown += `\n`;
            }
            // Pattern Drift
...
exports.generateRefactorBundleReport = generateRefactorBundleReport;
/**
 * Extract symbols from file content (simplified version for working tree analysis)
 */
async function extractSymbolsFromContent(content, filePath) {
    const { getTreeSitterParser, detectLanguage } = await Promise.resolve().then(() => __importStar(require('../analysis/tree-sitter')));
...
async function generateCommitReport(commitShas) {
    try {
        const { getDatabaseManager, ensureDatabaseInitialized } = await Promise.resolve().then(() => __importStar(require('../storage/database')));
        await ensureDatabaseInitialized();
        const db = getDatabaseManager().getDatabase();
    ...
    symbols_added, symbols_modified, symbols_removed, risks
            ...
            ORDER BY date DESC
    `);
            commits = commitsStmt.all(...commitShas);
        }
        else {
            // Fetch all commits
            const commitsStmt = db.prepare(`
    ...
    `);
                ...
                LIMIT 5
              `);
                            const calls = callsStmt.all(commit.sha, item.symbol_id);
                            if (calls.length > 0) {
        ...
        `);
                ...
                LIMIT 5
              `);
                            const calledBy = calledByStmt.all(commit.sha, item.symbol_id);
      ...
      `);
            const edgeCount = edgesStmt.get(commit.sha);
          ...
          LIMIT 5
        `);
                const topCallers = topCallersStmt.all(commit.sha);
              ...
              LIMIT 3
            `);
                        const calls = callsStmt.all(commit.sha, caller.from_symbol_id);
                        if (calls.length > 0) {
...
exports.generateCommitReport = generateCommitReport;
...
function extractSymbolName(symbolId) {
