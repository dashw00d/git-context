...
exports.buildIntendedMap = buildIntendedMap;
/**
 * Fallback: Build intended map from hotspots (high churn = divergent) and moved blocks (DNA continuity)
 */
async function buildIntendedMapFallback(commitShas, db) {
    const intended = new Map();
    ...
    LIMIT 100
  `);
    const hotspots = hotspotStmt.all(churnThreshold);
  ...
  `);
    const movedBlocks = movedStmt.all(...commitShas);
