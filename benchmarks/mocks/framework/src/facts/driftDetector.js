...
exports.detectDrift = void 0;
const database_1 = require("../storage/database");
const namingConventions_1 = require("../analysis/namingConventions");
/**
 * Detect divergent symbol clusters using AST shape similarity and reachability analysis
 */
function detectDivergentClusters(working, intended) {
    const divergentClusters = [];
    const suggestedConsolidations = [];
 ...
 */
function clusterByShape(symbols) {
    const clusters = [];
    // Group symbols by their DNA hash (if available)
    const dnaClusters = new Map();
    const symbolsWithoutDna = [];
 ...
 */
function clusterBySignature(symbols) {
    const clusters = [];
    for (const symbol of symbols) {
 ...
 */
function calculateSymbolSimilarity(a, b) {
 ...
 */
function calculateSignatureSimilarity(sigA, sigB) {
    if (!sigA || !sigB)
        return 0;
 ...
 */
function findReachableSymbols(working, intended) {
 ...
 */
function detectDrift(intended, working, commitShas) {
    const findings = {
        missing_symbols: [],
        zombie_symbols: [],
        divergent_symbols: [],
    ...
    if (commitShas && commitShas.length > 0) {
      ...
      `);
            const intendedEdges = intendedEdgesStmt.all(...commitShas);
            // Check for missing edges (intended present but not in working)
            for (const intendedEdge of intendedEdges) {
                const found = working.edges.find(e => e.from_symbol_id === intendedEdge.from_symbol_id &&
 ...
 * Detect unresolved callers (call sites that don't resolve to a current symbol)
 */
function detectUnresolvedCallers(working, intended) {
    const nameIndex = new Map(); // lowerName -> symbol_ids
    for (const [symbolId, symbol] of working.symbolsById) {
    ...
    for (const edge of working.edges) {
        ...
        if (edge.to_symbol_id && working.symbolsById.has(edge.to_symbol_id))
            continue;
...
function extractCalleeName(raw) {
    if (!raw)
        return '';
...
function guessTarget(calleeName, nameIndex, intended) {
    if (!calleeName)
        return null;
    ...
    if (intended) {
        const intendedMatch = Array.from(intended.entries()).find(([, st]) => st.lastName && st.lastName.toLowerCase() === calleeName.toLowerCase());
        if (intendedMatch) {
 ...
 */
function detectConventionDrift(working, commitShas) {
    try {
        ...
        const driftResult = (0, namingConventions_1.analyzeConventionDrift)(symbols);
        ...
        const mixedConventionFiles = [];
        for (const [filePath, fileSymbols] of symbolsByFile.entries()) {
            if (fileSymbols.length < 2)
                continue; // Need at least 2 symbols to have mixing
            const fileDrift = (0, namingConventions_1.analyzeConventionDrift)(fileSymbols);
            const uniqueConventions = new Set(fileSymbols.map(s => {
            ...
            if (uniqueConventions.size > 1 && fileDrift.driftPercent > 0) {
                mixedConventionFiles.push({
                    path: filePath,
                    conventions: Array.from(uniqueConventions),
