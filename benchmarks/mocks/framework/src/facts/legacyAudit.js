...
exports.auditLegacy = void 0;
/**
 * Scoped reachability analysis for dead/legacy/replaced detection
 */
async function auditLegacy(intended, working, scope) {
    ...
    if (intended.size === 0) {
      ...
      LIMIT 50
    `);
        const hotspotSymbols = hotspotStmt.all();
...
exports.auditLegacy = auditLegacy;
/**
 * Find entry points (roots) for reachability analysis
 */
function findEntryPoints(working, scope) {
    const roots = new Set();
    ...
    for (const [symbolId, symbol] of working.symbolsById) {
        ...
        if (filePathLower.includes('controller') && !scope.allPaths.has(filePath))
            continue;
        if (filePathLower.includes('service') && !scope.allPaths.has(filePath))
            continue;
        ...
        if (symbol.kind === 'function' || symbol.kind === 'method') {
            ...
            if (/^(main|run|start|init|setup|bootstrap|create|build)$/.test(name) ||
                ...
                ['handle', 'process', 'execute', 'render', 'mount', 'unmount', 'destroy'].some(pattern => name.includes(pattern))) {
                roots.add(symbolId);
                continue;
            }
        }
 ...
 */
function isInScope(symbolId, scope) {
    const filePath = symbolId.split(':')[0];
 ...
 */
function isLikelyUtilityFunction(symbol) {
    const name = symbol.name.toLowerCase();
 ...
 */
function isEntryPointLike(symbol) {
    const name = symbol.name.toLowerCase();
 ...
 */
function hasSomeComplexity(symbol) {
 ...
 */
function findReplacedLeftovers(intended, working, inboundGraph) {
    const leftovers = [];
    // Find symbols intended to be absent
    const absentSymbols = new Map();
    for (const [symbolId, expected] of intended) {
        if (expected.expect === 'absent') {
    ...
    for (const [absentId, expected] of absentSymbols) {
        ...
        if (inboundCount > 10)
            continue; // Too many callers, probably still in active use
        // Skip if the symbol is still being used by entry points
        const callers = inboundGraph.get(absentId) || new Set();
        const hasEntryPointCallers = Array.from(callers).some(callerId => working.symbolsById.has(callerId) && isEntryPointLike(working.symbolsById.get(callerId)));
        if (hasEntryPointCallers && inboundCount > 3)
            continue;
        // Find similar symbols in working tree
        const candidates = [];
        for (const [workingId, workingSymbol] of working.symbolsById) {
            if (workingId === absentId)
                continue;
 ...
 */
function calculateSimilarity(oldSymbol, newSymbol, expected) {
    let score = 0;
    let total = 0;
