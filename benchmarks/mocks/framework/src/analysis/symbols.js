...
class SymbolExtractor {
    ...
    async extractFileSymbols(sha, file) {
        ...
        try {
            ...
            const currentSymbolsWithDNA = (0, symbolDna_1.assignDNAIds)(currentSymbols, bodyTexts);
            ...
            if (previousContent) {
                ...
                previousSymbolsWithDNA = (0, symbolDna_1.assignDNAIds)(previousSymbols, bodyTexts);
            }
     ...
     */
    async extractWorkingTreeSymbols(files, options = {}) {
        const added = [];
        const removed = [];
        const modified = [];
        for (const file of files) {
     ...
     */
    async extractWorkingTreeFileSymbols(file, options) {
        const added = [];
        const removed = [];
        const modified = [];
        try {
            ...
            if (headContent) {
                ...
                headSymbolsWithDNA = (0, symbolDna_1.assignDNAIds)(headSymbols, headBodyTexts);
            }
            // Compare and categorize changes
            ...
            for (const delta of changes.modified) {
                ...
                if (headContent && currentContent) {
                    const snippets = this.semanticDetector.extractDiffSnippets(headContent, currentContent, delta.symbol);
                    delta.diffSnippetPre = snippets.pre;
                    delta.diffSnippetPost = snippets.post;
                }
            }
     ...
     */
    async extractIncremental(prevSymbols, changes, content, path) {
     ...
     */
    async extractSymbolsWithBodies(content, filePath, language) {
        ...
        for (const symbol of symbols) {
            const bodyText = this.extractBodyText(content, symbol.location.start.line, symbol.location.end.line);
            bodyTexts.set(symbol.id, bodyText);
        }
        return { symbols, bodyTexts };
    }
    extractBodyText(content, startLine, endLine) {
