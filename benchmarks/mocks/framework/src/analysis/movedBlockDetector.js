...
exports.MovedBlockDetector = void 0;
const logger_1 = require("../utils/logger");
const database_1 = require("../storage/database");
const git_1 = require("./git");
class MovedBlockDetector {
    constructor(dbManager = (0, database_1.getDatabaseManager)(), git) {
        this.dbManager = dbManager;
        this.git = git;
        this.similarityThreshold = 0.6; // Configurable threshold (default 0.6)
        if (!this.git) {
     ...
     */
    async detectMovedBlocks(commitSha, deletedSymbols, addedSymbols) {
        (0, logger_1.logInfo)(`[MovedBlockDetector] Detecting moves for commit ${commitSha.substring(0, 8)}`);
        ...
        const movedBlocks = candidates.map(candidate => ({
            commitSha,
        ...
        (0, logger_1.logInfo)(`[MovedBlockDetector] Found ${movedBlocks.length} moved blocks, ${symbolLineage.length} lineage entries`);
        return { movedBlocks, symbolLineage };
    }
    /**
     * Extract code blocks from symbols with content hashing
     */
    async extractCodeBlocks(symbols, commitSha, context) {
        const blocks = [];
        for (const symbol of symbols) {
            try {
                ...
                (0, logger_1.logDebug)(`[MovedBlockDetector] Failed to extract block for ${symbol.name}: ${error}`);
            }
        }
        return blocks;
    }
     ...
     */
    findMoveCandidates(deletedBlocks, addedBlocks) {
        const candidates = [];
        for (const deleted of deletedBlocks) {
            for (const added of addedBlocks) {
                ...
                // Stage 3: Fuzzy match (partial moves)
                    ...
                    similarityScore = this.calculateLevenshteinSimilarity(deleted.normalizedHash, added.normalizedHash);
                    if (similarityScore < this.similarityThreshold)
                        continue; // Too different
                }
                candidates.push({
    ...
    deduplicateCandidates(candidates) {
        ...
        for (const group of Array.from(grouped.values())) {
            const best = group.reduce((best, current) => current.similarityScore > best.similarityScore ? current : best);
     ...
     */
    async storeMovedBlocks(movedBlocks) {
      ...
      `);
            stmt.run(block.commitSha, block.sourceFile, block.sourceSymbolId || null, block.sourceStartLine, block.sourceEndLine, block.sourceContentHash, block.destFile, block.destSymbolId || null, block.destStartLine, block.destEndLine, block.destContentHash, block.similarityScore, block.blockType, block.moveReason, block.lineCount);
        }
    }
    /**
     * Store symbol lineage in database
     */
    async storeSymbolLineage(lineage) {
        const db = this.dbManager.getDatabase();
    ...
    async getFileContent(filePath, commitSha) {
        if (!this.git) {
            (0, logger_1.logDebug)(`[MovedBlockDetector] GitOperations not available, returning empty content`);
            return '';
        }
        try {
            ...
            (0, logger_1.logDebug)(`[MovedBlockDetector] Failed to get file content for ${filePath} at ${commitSha.substring(0, 8)}: ${error.message}`);
            return '';
        }
    }
    /**
     ...
     */
    extractSymbolContent(fileContent, symbol) {
    ...
    hasMultipleMovesToSameFile(filePath) {
    ...
    isFileRename(sourcePath, destPath) {
     ...
     */
    async getMovedBlocks(commitSha) {
        const db = this.dbManager.getDatabase();
        const stmt = db.prepare(`
     ...
     */
    async getSymbolLineage(symbolId) {
        const db = this.dbManager.getDatabase();
        const stmt = db.prepare(`
...
exports.MovedBlockDetector = MovedBlockDetector;
