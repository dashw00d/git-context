...
exports.auditAllModules = exports.migrateDatabase = exports.ANALYSIS_VERSION = exports.MODULE_SCHEMAS = exports.safeAddColumn = void 0;
// === SAFE COLUMN HELPER ===
/**
...
exports.ANALYSIS_VERSION = '2.0';
// === MIGRATION RUNNER ===
/**
 * Truncate error message to prevent log spam
 */
function truncateError(msg, maxLen = 200) {
    if (!msg)
...
function migrateDatabase(db) {
    ...
    try {
        db.exec(exports.MODULE_SCHEMAS.core.schema);
    }
    catch (error) {
    ...
    const moduleOrder = ['core', 'commits', 'symbols', 'edges', 'conventions', 'structural', 'hotspots', 'moved', 'reports'];
    for (const modName of moduleOrder) {
        const mod = exports.MODULE_SCHEMAS[modName];
        try {
            // Get current version for this module
            const maxVerResult = db.prepare('SELECT MAX(version) as maxVer FROM migration_log WHERE module = ?').get(modName);
            let currentVer = 0;
            if (maxVerResult) {
...
function auditAllModules(db) {
    ...
    try {
        const legacyResult = db.prepare('SELECT COUNT(*) as cnt FROM edges WHERE edge_type IS NULL OR edge_type = ""').get();
...
exports.auditAllModules = auditAllModules;
