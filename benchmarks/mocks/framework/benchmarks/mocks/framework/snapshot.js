...
exports.applySnapshotToState = exports.loadStepSnapshot = exports.saveStepSnapshot = exports.serializeStepState = exports.deepSerializeStepState = exports.stableHash = void 0;
 ...
 */
function deepSerializeStepState(stepId, state) {
    if (!state)
        return null;
    const seen = new WeakSet();
    const MAX_STRING_LENGTH = 2000;
    const deepSerialize = (value, depth = 0) => {
        ...
        if (llmAnalysis) {
            return {
                llmAnalysis: {
                    ...
                    blocks: llmAnalysis.blocks ? llmAnalysis.blocks.map((block) => ({
                        id: block.id,
                        title: block.title,
                        type: block.type,
...
exports.deepSerializeStepState = deepSerializeStepState;
...
function applySnapshotToState(state, snapshot) {
    ...
    switch (stepId) {
        case 'scope':
            ...
            break;
        case 'intended':
            state.intended = data ? new Map(data.map(item => [item.symbolId, {
                    expect: item.expect,
        ...
        case 'working':
            state.working = data ? {
                symbolsById: new Map((data.symbols || []).map((s) => [s.symbolId, {
                        id: 0,
